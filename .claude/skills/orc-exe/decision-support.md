...
This skill helps the ORC-EXE (and the operator) decide when to use manual control versus autonomous execution for different types of actions. It analyzes complexity, risk, and context to recommend the appropriate execution mode.
...
```
orc-exe → decision-support.md → recommendation (manual/autonomous)
```
...
ORC-EXE: [Invokes decision-support.md]
