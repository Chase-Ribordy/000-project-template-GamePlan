...
The Pass Awareness Advisor is an intelligence layer that helps orc-exe understand WHERE the project is in the development lifecycle and WHAT approaches are appropriate for that phase. This skill provides soft recommendations and contextual nudges—it's an advisor, not a dictator.
...
### Writes:
- Returns advisory messages (string/markdown) to orc-exe
- No direct file modifications
- All output is advisory, consumed by orc-exe for decision-making
...
```
ORC-EXE: "I'm planning the next batch of stories. What pass context should I consider?"
...
```
ORC-EXE: "All first-pass stories are done. What's next?"
...
```
ORC-EXE: "Should I use manual or autonomous for story #15?"
...
[ORC-EXE then consults decision-support.md for final mode decision]
```

### Pattern 4: Frustration Response
```
ORC-EXE: "Operator signals: 'This is taking too long, just build it.'"
...
```mermaid
graph TD
    A[ORC-EXE: Plan Stories] --> B[Invoke pass-awareness-advisor]
    B --> C{Pass Detection}
...
Pass Awareness Advisor makes orc-exe intelligent about development phases without being bossy. It provides:
