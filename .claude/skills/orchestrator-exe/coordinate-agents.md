...
This skill enables the Orchestrator-EXE to coordinate BMAD agents (Dev, SM, Architect, etc.) through direct delegation rather than event-driven communication. It manages the handoff of work items, tracks agent assignments, and maintains coordination history.

## Invocation Pattern

```
orchestrator-exe → coordinate-agents.md → BMAD agent
```

Unlike autonomous BMAD skills that emit/listen to events, this skill is directly invoked by orchestrator-exe and operates synchronously.
...
Orchestrator-EXE: [Loads coordinate-agents.md skill]
...
7. Return control to orchestrator-exe when agent completes

Result: Story implemented, orchestrator-exe suggests next action
```
...
4. **Top-Down Flow**: orchestrator-exe → skill → agent (never reversed)
...
7. **Track completion**: Return to orchestrator-exe and update coordination-log.yaml

### Example: Component Integration Assignment
```
Orchestrator-EXE identifies task: "Integrate login-form component"
