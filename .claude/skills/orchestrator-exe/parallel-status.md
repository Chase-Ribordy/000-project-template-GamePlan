...
```
orchestrator-exe → *parallel-status → formatted status output
```
...
| **User** | Any agent following BMAD method | Orchestrator-EXE only |
...
- ✓ Skill file created at `.claude/skills/orchestrator-exe/parallel-status.md`
- ✓ Isolation level declared (Orchestrator-EXE only)
