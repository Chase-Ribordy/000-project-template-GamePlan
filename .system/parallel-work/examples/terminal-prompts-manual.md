...
This document shows copy-paste ready terminal prompts generated by orc-exe for manual parallel work coordination. These examples demonstrate the `terminal-prompt-generator` skill output.
...
COORDINATION NOTES:
  - ⚠️  You will create the users table schema
  - ⚠️  Terminal 3 (audit logging) is WAITING for your schema
  - 📢 When users table is complete, notify orchestrator
...
Status: ⏸️  PAUSED - Resume when T1 completes schema
...
COORDINATION NOTES:
  - ⚠️  WAIT for Terminal 1 to create users table
  - 📢 Estimated resume time: ~30 minutes from now
...
COORDINATION NOTES:
  - 📝 Create payment-gateway-contract.md (Markdown format)
  - 🤖 Orchestrator will convert to YAML for MCP server
  - ⚠️  Terminal 2 will implement webhook handler - coordinate event types
  - 📢 Share Stripe event types list with Terminal 2 when defined
...
Status: ▶️  START IMMEDIATELY (can work in parallel)
...
COORDINATION NOTES:
  - ⚠️  Coordinate with Terminal 1 on Stripe event types
  - Wait for T1's event type list (estimated 30 min) before finalizing handlers
  - 📝 Create webhook-handler-contract.md
  - 🤖 Both contracts will be validated together by MCP server
...
COORDINATION NOTES:
  - ⚠️  Shares Dashboard component with T2 and T3
  - 📝 Use DashboardChartsSection.tsx (new component)
  - 🔄 Coordinate merge with T2 (layout) and T3 (filters)
  - 📢 Notify when component interface defined (~1 hour)
...
COORDINATION NOTES:
  - ⚠️  Defines main Dashboard.tsx structure
  - 📝 Create layout slots for T1 (charts) and T3 (filters)
  - 🔄 T1 and T3 will plug into your layout
  - 📢 Share layout prop interface early (~30 min)
...
COORDINATION NOTES:
  - ⚠️  Integrates with T1 (filters affect chart data)
  - ⚠️  Plugs into T2's layout
  - 📝 Use DashboardFilters.tsx (new component)
  - 🔄 Share filter state interface with T1
  - 📢 Notify when filter interface ready (~45 min)
...
Integration Phase (Final Hour):
  ...
  2. Wire up filter → chart data flow
...
**Mode Transition**: Manual → Autonomous after first story
...
═══════════════════════════════════════════════════════════════════════════════
Status: ⏸️  WAITING for Story 4-1 pattern completion
...
Decision Point: Continue autonomous or revert to manual?
  - If pattern is clear and tests pass → Autonomous
  - If pattern needs adjustment → Stay manual
...
1. Operator invokes `/orc-exe` after sprint planning
